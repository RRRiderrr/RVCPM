(() => {
  ...
  function renderState(){
    ...
    $('#restartBanner').classList.toggle('hidden',!state.pendingRestart);
  ...
  function renderPlugins(){
    ...
    (state.plugins||[]).forEach((p,i)=>{
      ...
      card.innerHTML=`${p.updateAvailable?`<div class="update-badge">${esc(tr('updateAvailable'))}</div>`:''}
        ...
        <div class="plugin-footer">${p.required?`<span class="plugin-meta-pill">${esc(tr('required'))}</span>`:''}<span class="plugin-meta-pill">${p.settingsCount||0} ${esc(tr('settingsCount'))}</span>${p.dependencies?.length?`<span class="plugin-meta-pill">deps ${p.dependencies.length}</span>`:''}<span class="spacer"></span>${p.hasSettings?`<button class="icon-btn settings-btn" title="${esc(tr('pluginSettings'))}">⚙</button>`:''}<button class="icon-btn details-btn" title="${esc(tr('details'))}">…</button></div>`;
  ...
  function openCandidateModal(batch){
    ...
    $('#installCandidates').addEventListener('click',async()=>{
      ...
      closeModal();try{await rpc('installCandidates',{batchId:batch.batchId,candidateIds:ids});}catch(err){toast(err.message,'error')}
    });
  }

  async function openSettings(p){
    try{
      const data=await rpc('getPluginSettings',{pluginId:p.id}), settings=data.settings||[];
      let body='';
      if(!settings.length)body=`<div class="unsupported-setting">${esc(tr('noConfigurableSettings'))}</div>`;
      else body=settings.map(s=>settingHtml(s)).join('');
      openModal(`<div class="modal-head"><div><div class="eyebrow">${esc(p.name)}</div><h2>${esc(tr('pluginSettings'))}</h2><p>${esc(p.pluginDescription||p.description||'')}</p></div><button class="modal-close" data-close>×</button></div><div class="modal-body"><div class="settings-form">${body}</div></div><div class="modal-foot"><button class="btn ghost" data-close>${esc(tr('cancel'))}</button><button class="btn primary" id="savePluginSettings">${esc(tr('save'))}</button></div>`);
      ...
      $('#savePluginSettings').addEventListener('click',async()=>{
        const values={};
        settings.forEach(s=>{
          if(s.unsupported)return;const el=document.querySelector(`[data-setting-key="${cssEscape(s.key)}"]`);if(!el)return;
          ...
          else if(s.type==='Custom'){try{values[s.key]=JSON.parse(el.value)}catch{throw new Error(`${s.displayName}: invalid JSON`);}}
  ...
  function settingHtml(s){
    const title=esc(s.displayName||s.key),desc=esc(s.description||'');
    if(s.unsupported)return `<div class="plugin-setting"><div class="plugin-setting-title">${title}</div><div class="plugin-setting-desc">${desc}</div><div class="plugin-setting-control"><div class="unsupported-setting">${esc(tr('settingsUnavailable'))} <b>${esc(s.type)}</b></div></div></div>`;
    const key=esc(s.key);let control='';
    if(s.type==='Boolean')control=`<input data-setting-key="${key}" class="toggle" type="checkbox" ${s.value===true?'checked':''}/>`;
    else if(s.type==='Number')control=`<input data-setting-key="${key}" class="text-input" type="number" value="${esc(s.value??s.defaultValue??0)}"/>`;
    else if(s.type==='Select')control=`<select data-setting-key="${key}" class="select">${(s.options||[]).map((o,i)=>`<option value="${i}" ${deepEqual((o.Value??o.value),s.value)?'selected':''}>${esc(o.Label??o.label??String(o.Value??o.value))}</option>`).join('')}</select>`;
    else if(s.type==='Slider'){const marks=s.markers||[],min=marks.length?Math.min(...marks):0,max=marks.length?Math.max(...marks):100,val=s.value??s.defaultValue??min;control=`<div class="slider-wrap"><input data-setting-key="${key}" class="slider-control" type="range" min="${min}" max="${max}" step="${marks.length>1?Math.max(.01,(max-min)/100):1}" value="${esc(val)}"/><span class="slider-value">${esc(val)}</span></div>`;}
    else if(s.type==='Custom')control=`<textarea data-setting-key="${key}">${esc(JSON.stringify(s.value??s.defaultValue??{},null,2))}</textarea>`;
    else if(s.multiline)control=`<textarea data-setting-key="${key}">${esc(s.value??s.defaultValue??'')}</textarea>`;
    else control=`<input data-setting-key="${key}" class="text-input" value="${esc(s.value??s.defaultValue??'')}"/>`;
    return `<div class="plugin-setting"><div class="plugin-setting-head"><div><div class="plugin-setting-title">${title}</div><div class="plugin-setting-desc">${desc}</div></div>${s.type==='Boolean'?control:''}</div>${s.type==='Boolean'?'':`<div class="plugin-setting-control">${control}</div>`}</div>`;
  }

  function openDetails(p){
    const readme=p.readme?markdown(p.readme):`<span style="color:var(--muted)">${esc(tr('noReadme'))}</span>`;
    openModal(`<div class="modal-head"><div><div class="eyebrow">${esc(sourceLabel(p))}</div><h2>${esc(p.name)}</h2><p>${esc(p.version||'')} · ${esc(p.target||'')}</p></div><button class="modal-close" data-close>×</button></div><div class="modal-body"><p class="details-desc">${esc(p.description||p.pluginDescription||'')}</p>${p.pluginDescription&&p.pluginDescription!==p.description?`<div class="notice"><b>${esc(tr('pluginDescription'))}</b><span>${esc(p.pluginDescription)}</span></div>`:''}<div class="modal-meta"><span class="plugin-meta-pill">${esc(tr(p.enabled?'enabled':'disabled'))}</span><span class="plugin-meta-pill">${p.settingsCount||0} ${esc(tr('settingsCount'))}</span><span class="plugin-meta-pill">${esc(sourceLabel(p))}</span></div><h3 style="margin-top:16px;font-size:12px">${esc(tr('readme'))}</h3><div class="readme-box">${readme}</div></div><div class="modal-foot"><button class="btn danger ghost" id="removePluginBtn">${esc(tr('remove'))}</button><span style="flex:1"></span><button class="btn ghost" id="openSourceBtn">${esc(tr('openSource'))}</button>${p.githubUrl?`<button class="btn ghost" id="openGithubBtn">GitHub ↗</button>`:''}${p.hasSettings?`<button class="btn primary" id="detailsSettingsBtn">${esc(tr('pluginSettings'))}</button>`:''}</div>`);
  ...
  function openRemoveModal(p){
    ...
    $('#confirmRemove').addEventListener('click',async()=>{const removeSettings=$('#removeSettingsCheck').checked;closeModal();try{await rpc('removePlugin',{pluginId:p.id,removeSettings})}catch(err){toast(err.message,'error')}});
  }
  ...
  function toast(message,kind='info'){const d=document.createElement('div');d.className=`toast ${kind}`;d.innerHTML=`<span>${esc(message)}</span>`;$('#toastRoot').appendChild(d);setTimeout(()=>{d.style.opacity='0';d.style.transform='translateX(12px)';setTimeout(()=>d.remove(),220)},4200)}
  ...
  $('#restartDiscordBtn').addEventListener('click',()=>withToast(()=>rpc('restartDiscord'),false));
